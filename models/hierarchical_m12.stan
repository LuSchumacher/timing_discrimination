...
parameters {
  ...
  real                           mu_bv;      // mean drift beta
  real<lower=0>                  sigma_bv;   // SD drift beta
  real<lower=0, upper=1>         mu_w;       // mean drift weight
  real<lower=0>                  sigma_w;    // mean drift weight
  ...
  real                           bv[S];      // individual drift beta 1 
  real<lower=0, upper=1>         w[S];       // individual drift weight
...
transformed parameters {
  ...
  for (i in 1:T){
    delta[i] = v0[sub[i]] + bv[sub[i]] * (w[sub[i]]*I[i] - (1-w[sub[i]])*d2[i]);
  }
...
model {
  ...
  for (i in 1:S) {
    ...
    bv[i]    ~ normal(mu_bv, sigma_bv);
    w[i]     ~ normal(mu_w, sigma_w)T[0,1];
